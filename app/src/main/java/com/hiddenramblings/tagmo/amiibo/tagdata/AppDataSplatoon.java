...
public class AppDataSplatoon extends AppData {
    ...
    public boolean checkSaveData(byte[] tagData) {
        byte[] saveData = new byte[gameDataBytes.length];
        System.arraycopy(tagData, 0xDC, saveData, 0, gameDataBytes.length);
        return Arrays.equals(gameDataBytes, saveData)
                || Arrays.equals(gameDataBytes2, saveData);
    }
    ...
    public void injectSaveData(boolean source) {
        byte[] appDataBytes = source
                ? TagArray.hexToByteArray(appDataHex.replace(" ", ""))
                : TagArray.hexToByteArray(appDataHex2.replace(" ", ""));
