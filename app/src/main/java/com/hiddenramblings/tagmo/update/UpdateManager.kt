...
class UpdateManager internal constructor(activity: BrowserActivity) {
    ...
    private fun getPlayUpdateManager(): AppUpdateManager {
        return appUpdateManager ?: AppUpdateManagerFactory.create(browserActivity).also {
            appUpdateManager = it
        }
    }

    private fun configurePlay() {
        getPlayUpdateManager().appUpdateInfo.addOnSuccessListener { appUpdateInfo: AppUpdateInfo ->
    ...
    fun resumePlayUpdateIfNeeded() {
        ...
        getPlayUpdateManager().appUpdateInfo.addOnSuccessListener { appUpdateInfo ->
    ...
    private fun clearUpdateStatus() {
        if (!isUpdateAvailable && null == updateUrl && null == appUpdate) return
        isUpdateAvailable = false
        updateUrl = null
        appUpdate = null
        updateListener?.onUpdateCleared()
    }
