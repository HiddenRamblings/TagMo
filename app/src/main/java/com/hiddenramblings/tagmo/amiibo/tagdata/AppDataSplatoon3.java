...
public class AppDataSplatoon3 extends AppData {
    static final int GAME_DATA_OFFSET = 0x0;

    public AppDataSplatoon3(byte[] appData) throws IOException {
        super(appData);
    }

    String saveDataHex =                        "01 01 00 00" +
            ...
            "88 9C B6 3D";
    byte[] saveDataBytes = TagArray.hexToByteArray(saveDataHex.replace(" ", ""));

    String saveDataHex2 =                       "01 01 00 00" +
            ...
            "AA 42 4E 6D";
    byte[] saveDataBytes2 = TagArray.hexToByteArray(saveDataHex2.replace(" ", ""));

    public boolean checkSaveData() {
        byte[] saveData = new byte[saveDataBytes.length];
        System.arraycopy(appData.array(), GAME_DATA_OFFSET, saveData, 0, saveDataBytes.length);
        return Arrays.equals(saveDataBytes, saveData) || Arrays.equals(saveDataBytes2, saveData);
    }

    public void injectSaveData(boolean source) {
        byte[] appDataBytes = source ? saveDataBytes : saveDataBytes2;
