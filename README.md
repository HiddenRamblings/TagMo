...
* Write dump files from legitimate Amiibos into an NTAG215 NFC in a way that WiiU/3DS devices will consider to be a legitimate Amiibo with the same functionality.
...
* Currently only Skylander Amiibos are supported 
...
* Use load tag to load an amiibo dump file, scan tag to scan an Amiibo.
...
# Editing Super Smash Bros (SSB) data (Beta)  
* Click "Scan Tag" to scan amiibo.
* Click "Edit SSB Data" to edit properties related to SSB. Using this on non SSB amiibo will corrupt its saved data. requiring you to restore it from a save file (if you have one) or to reset it. It is recommended you back the amiibo data using the steps above before trying this out.
