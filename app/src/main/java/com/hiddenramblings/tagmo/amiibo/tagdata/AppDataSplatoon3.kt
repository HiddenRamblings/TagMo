...
class AppDataSplatoon3(appData: ByteArray?) : AppData(appData!!) {
    var saveDataHex = "01 01 00 00" +
            ...
            "88 9C B6 3D"
    var saveDataBytes = TagArray.hexToByteArray(saveDataHex.replace(" ", ""))
    var saveDataHex2 = "01 01 00 00" +
            ...
            "AA 42 4E 6D"
    var saveDataBytes2 = TagArray.hexToByteArray(saveDataHex2.replace(" ", ""))
    fun checkSaveData(): Boolean {
        val saveData = ByteArray(saveDataBytes.size)
        System.arraycopy(appData.array(), GAME_DATA_OFFSET, saveData, 0, saveDataBytes.size)
